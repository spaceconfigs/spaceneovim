...
local commander = require("domain.contracts.controller_registry").commander()
local commenter = require("domain.contracts.controller_registry").commenter()
local terminal = require("domain.contracts.controller_registry").terminal()
...
return function()
	---@type KeyBind[]
	local default_map = {
		{ key = "<leader>", description = "Commands", method = commander.find() },
		{ key = "1", description = "Window 1", method = window.go_to({ number = 1 }) },
		{ key = "2", description = "Window 2", method = window.go_to({ number = 2 }) },
		{ key = "3", description = "Window 3", method = window.go_to({ number = 3 }) },
		{ key = "4", description = "Window 4", method = window.go_to({ number = 4 }) },
		{ key = "5", description = "Window 5", method = window.go_to({ number = 5 }) },
		{ key = "6", description = "Window 6", method = window.go_to({ number = 6 }) },
		{ key = "7", description = "Window 7", method = window.go_to({ number = 7 }) },
		{ key = "8", description = "Window 8", method = window.go_to({ number = 8 }) },
		{ key = "9", description = "Window 9", method = window.go_to({ number = 9 }) },
		{ key = ";", description = "Comment line", method = commenter.toggle_current_line() },
		{
			key = ";",
			description = "Comment line",
			method = commenter.toggle_current_line_visual_mode(),
			mode = "v",
		},
		{ key = "<Tab>", description = "Previous buffer", method = buffer.reopen() },
		{
			key = "'",
			mode = { "n" },
			description = "Terminal",
			method = terminal.toggle(),
		},
		{
			key = '"',
			mode = { "n", "t" },
			description = "External terminal",
			method = terminal.toggle({ type = "external" }),
		},
	}
