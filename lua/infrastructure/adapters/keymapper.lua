...
local format_group = function(map)
	return {
		...
		mode = map.mode,
		-- Buffer-local groups (per-filetype menus) must stay scoped to their
		-- buffer; without this a group label registers globally and its scoping
		-- depends on which-key pruning childless nodes rather than on intent.
		buffer = map.buffer,
		hidden = map.hidden,
	}
end
