...
local M = {
  ...
  start_highlighting = function(buffer)
    ...
    local available, added = pcall(vim.treesitter.language.add, language)
    if not (available and added) then
      return true
    end
  ...
  filetype = function(buffer)
    return vim.bo[buffer or 0].filetype
  end,
