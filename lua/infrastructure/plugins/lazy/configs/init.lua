return {
        ...
        win = {
          input = {
            keys = {
              ["<c-b>"] = false,
              ["<c-f>"] = false,
              ["<C-A-v>"] = { "preview_scroll_down", mode = { "i", "n" } },
              ["<C-A-S-v>"] = { "preview_scroll_up", mode = { "i", "n" } },
            },
          },
          list = {
            keys = {
              ["<c-b>"] = false,
              ["<c-f>"] = false,
              ["<C-A-v>"] = "preview_scroll_down",
              ["<C-A-S-v>"] = "preview_scroll_up",
            },
          },
          preview = {
            keys = {
  ...
  -- Local checkout, so changes in ~/Projects/nvim-lspconfig are picked up directly.
  -- Drop `dir` to go back to the upstream release.
            ...
            notifications = {
              {
                ...
                fn = function(picker, item)
                  require("octo.notifications").request_read_notification(item.id)
                  picker:close()
                  require("octo.picker").notifications()
                end,
              },
              {
                name = "mark_notification_done",
                lhs = "<localleader>nd",
                desc = "mark notification as done",
                mode = { "n", "i" },
                fn = function(picker, item)
                  require("octo.notifications").delete_notification(item.id)
                  picker:close()
                  require("octo.picker").notifications()
                end,
              },
              {
                ...
                lhs = "<localleader>nu",
                desc = "unsubscribe from notification",
                mode = { "n", "i" },
                fn = function(picker, item)
                  require("octo.notifications").unsubscribe_notification(item.id)
                  picker:close()
                  require("octo.picker").notifications()
                end,
              },
