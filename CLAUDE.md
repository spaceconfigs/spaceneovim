...
- **Domain** (`lua/domain/`) — Pure data: keybinding definitions (`maps/`), enums (`environments.lua`, `log_levels.lua`), contracts (`contracts/`, including the controller registry seam `contracts/controller_registry.lua` and hook registry seam `contracts/hook_registry.lua`). No external dependencies, no `vim.*`.
- **Application** (`lua/application/`) — Use cases (`usecases/`) orchestrate business logic, ports (`ports/`) expose registries (`ports/adapter_registry.lua` for adapters, `ports/usecase_registry.lua` for use cases), helpers (`helpers/`) provide logging proxies.
...
Dependency direction: Domain ← Application ← { Presentation, Infrastructure }. Each layer's `init.lua` wires its concrete modules into a registry seam as lazy resolvers, so inner layers never import outer layers directly: controllers → `domain/contracts/controller_registry.lua`, hooks → `domain/contracts/hook_registry.lua`, adapters → `application/ports/adapter_registry.lua`, use cases → `application/ports/usecase_registry.lua`, plugin wrappers → `infrastructure/plugins/plugin_registry.lua`.
...
**Use case** (`application/usecases/foo.lua`): Exposes `M.setup()` that resolves the adapter via `require("application.ports.adapter_registry").foo()`, executes business logic directly (no currying), and is wrapped in `make_logged`. Never requires infrastructure directly.
...
1. Define the controller contract in `lua/domain/contracts/<feature>.lua` and add its resolver field to `domain/contracts/controller_registry.lua`
2. Define keybindings in `lua/domain/maps/<feature>.lua` (resolve controller via `domain.contracts.controller_registry`)
3. Create controller in `lua/presentation/controllers/<feature>.lua` (currying, use case via `usecase_registry`)
4. Create use case in `lua/application/usecases/<feature>.lua` (business logic, adapter via `ports.adapter_registry`)
